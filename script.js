...
function resetVoice(button) {
  ...
  const handle = card?.querySelector('.voice-progress-handle');
...
function seekFromHandlePointer(event, progressElement) {
  if (!voiceDemoPlayer || !activeVoiceButton) return;
  ...
  voiceDemoPlayer.currentTime = ratio * duration;
  ...
  if (timeLabel) {
    timeLabel.textContent =
      `${fmt(voiceDemoPlayer.currentTime)} / ${fmt(duration)}`;
  }
}

document.querySelectorAll('.voice-progress-handle').forEach(handle => {
  const progress = handle.closest('.voice-progress');
  let dragging = false;
  ...
  handle.addEventListener('pointerdown', event => {
    ...
    const card = handle.closest('.voice-demo-card');
    const button = card?.querySelector('.voice-play-btn');

    if (button && activeVoiceButton !== button) {
      button.click();
    }

    if (!voiceDemoPlayer || !Number.isFinite(voiceDemoPlayer.duration)) return;

    dragging = true;
    wasPlayingBeforeDrag = !voiceDemoPlayer.paused;

    handle.classList.add('is-dragging');
    handle.setPointerCapture?.(event.pointerId);
    seekFromHandlePointer(event, progress);
  });
  ...
  function stopDragging(event) {
    ...
    if (wasPlayingBeforeDrag && voiceDemoPlayer.paused) {
      voiceDemoPlayer.play().catch(() => {});
    }
  }
  ...
  handle.addEventListener('keydown', event => {
    ...
    if (event.key === 'ArrowRight') {
      event.preventDefault();
      voiceDemoPlayer.currentTime = Math.min(
        voiceDemoPlayer.duration,
        voiceDemoPlayer.currentTime + step
      );
    } else if (event.key === 'ArrowLeft') {
      event.preventDefault();
      voiceDemoPlayer.currentTime = Math.max(
        0,
        voiceDemoPlayer.currentTime - step
      );
      ...
      event.preventDefault();
      voiceDemoPlayer.currentTime = voiceDemoPlayer.duration;
    }
  });
});
    ...
    if (progressBar && Number.isFinite(duration) && duration > 0) {
      ...
      if (progress) {
        ...
        progress.setAttribute(
          'aria-valuetext',
          `${fmt(current)} จาก ${fmt(duration)}`
        );
      }

      if (handle && !handle.classList.contains('is-dragging')) {
